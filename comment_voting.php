...
while($comment = $stmt->fetch(PDO::FETCH_ASSOC)) { 
    ...
    $stmt->execute(array(
        ...
        ':reciever' => $request->comment_id,
    ));
    header('Location: ' . $_SERVER['HTTP_REFERER']);
}

$stmt = $__db->prepare("SELECT * FROM comment_likes WHERE sender = :username AND reciever = :reciever AND type = 'd'");
$stmt->bindParam(":username", $_SESSION['siteusername']);
$stmt->bindParam(":reciever", $request->comment_id);
$stmt->execute();
while($comment = $stmt->fetch(PDO::FETCH_ASSOC)) { 
...
if($request->action == "l") {
    ...
    $stmt = $__db->prepare("INSERT INTO comment_likes (sender, reciever, type) VALUES (:sender, :reciever, 'd')");
    $stmt->bindParam(":sender", $_SESSION['siteusername']);
    $stmt->bindParam(":reciever", $request->comment_id);
    $stmt->execute();
}
